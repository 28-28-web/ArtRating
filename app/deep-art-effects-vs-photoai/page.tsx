    ...
    <main className="mx-auto flex w-full max-w-3xl flex-1 flex-col gap-8 px-6 py-16">
      ...
      <div className="overflow-x-auto rounded-2xl border border-border-soft">
        <table className="w-full text-left text-sm">
          <thead className="bg-[var(--border-soft)]/30">
            <tr>
              <th className="p-3 font-medium text-ink">&nbsp;</th>
              <th className="p-3 font-medium text-ink">{dae.name}</th>
              <th className="p-3 font-medium text-ink">{pai.name}</th>
            </tr>
          </thead>
          <tbody>
            {rows.map(([label, a, b]) => (
              <tr key={label} className="border-t border-border-soft">
                <td className="p-3 font-medium text-ink-soft">{label}</td>
                <td className="p-3 text-ink">{a}</td>
                <td className="p-3 text-ink">{b}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        {[dae, pai].map((tool) => (
          <a
            ...
            className="rounded-xl bg-ink px-4 py-3 text-center font-medium text-canvas hover:opacity-90"
          >
            Try {tool.name} →
          </a>
        ))}
      ...
      <div>
        <h2 className="font-display text-lg font-semibold text-ink">Verdict</h2>
